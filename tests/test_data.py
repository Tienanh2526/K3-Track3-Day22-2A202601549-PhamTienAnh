...
def test_split_has_no_prompt_leakage() -> None:
    examples = load_jsonl("data/sample_preferences.jsonl")
    train, val = split_by_prompt(examples, validation_ratio=0.3, seed=7)
...
def test_load_jsonl_detects_duplicate_rows(tmp_path: Path) -> None:
    row = '{"prompt": "a", "chosen": "b", "rejected": "c"}\n'
